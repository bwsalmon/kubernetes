...
func (f *frameworkImpl) computeBatchablePlugins() error {
	...
	plugins := map[string]fwk.BatchablePlugin{}
...
func (f *frameworkImpl) GetNodeHint(ctx context.Context, pod *v1.Pod, state fwk.CycleState, nodeInfos fwk.NodeInfoLister, cycleCount int64) (hint string, signature string) {
	return f.batch.GetNodeHint(ctx, pod, state, nodeInfos, cycleCount)
}
...
func (f *frameworkImpl) PodSucceeded(ctx context.Context) {
	f.batch.PodSucceeded(ctx)
}
