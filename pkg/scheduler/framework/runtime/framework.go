...
type frameworkImpl struct {
	...
	batchablePlugins     []framework.BatchablePlugin
...
func NewFramework(ctx context.Context, r Registry, profile *config.KubeSchedulerProfile, opts ...Option) (framework.Framework, error) {
	...
	f.enableBatching = profile.EnableBatching
...
func (f *frameworkImpl) checkPluginSignatures() {
	for _, pl := range f.preFilterPlugins {
		if _, implements := pl.(framework.BatchablePlugin); !implements && f.enableBatching {
...
func (f *frameworkImpl) SignPod(ctx context.Context, pod *v1.Pod) (string, error) {
	if !f.enableBatching {
		return framework.Unsignable, nil
	}
	...
	for _, pl := range f.batchablePlugins {
		...
		if err != nil {
			return framework.Unsignable, err
		}
